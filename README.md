...
- 만료 시간이 지날 때 까지 갱신되지 않은 레코드를 자동 정리
...
- `RecordExpirationTask`가 30분마다 실행되어, 마지막 갱신 후 만료시간 이상 경과한 레코드를 삭제
...
클라이언트를 중지하면 더 이상 갱신 요청이 발생하지 않습니다. 마지막 갱신 후 **만료시간 이상 경과 (기본값 1시간)**하면 서버의 정리 태스크가 해당 레코드를 삭제하며, 서버 로그에 다음과 같이 기록됩니다.

...
3. **DNS Zone(DNS 영역)** 리소스를 생성합니다. (Private DNS 아님)
   - 3.1. DNS는 Cloudflare 등에서 서브 도메인을 생성할 수 있는 최상위 도메인으로 생성합니다.
   - 3.2. 이름에 최상위 DNS를 입력한 뒤, DNS 영역 편집기에 아래와 같은 식으로 입력하고 생성합니다.

```text
@	3600	IN	SOA	kallie.ns.cloudflare.com. dns.cloudflare.com. 2053518996 10000 2400 604800 3600

;; NS Records
@	86400	IN	NS	kallie.ns.cloudflare.com.
@	86400	IN	NS	kellen.ns.cloudflare.com.
```

   - 3.3. NS Record를 수정합니다. (`kallie.ns.cloudflare.com` 등 이름 서버 추가)
